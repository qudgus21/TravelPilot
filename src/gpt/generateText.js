...
export async function generateCityIntro(cityName, title) {
  ...
  [마무리]
  - 마지막 문장은 이 도시의 위치나 분위기, 여행자가 머물며 겪게 될 특성을 정리하는 느낌으로 마무리해주세요.
  - "숙소를 소개합니다" 같은 직접적인 표현은 사용하지 말고, 도시 자체의 특성을 정보 중심으로 자연스럽게 정리해주세요.
  `;
...
export async function generateHotelDescription(hotel) {
  ...
  [목적]
  아래 숙소 정보를 바탕으로, 실제 블로그에 게재할 수 있을 만큼 자연스럽고 신뢰감 있는 숙소 소개 문장을 작성해주세요.
  광고 같지 않고, 실제 경험에 기반한 후기처럼 읽히는 글이 좋습니다.
  ...
  - "선사합니다", "되찾을 수 있습니다", "풍성하게 만들어보세요" 같은 번역체 표현은 사용하지 마세요.
  - 실제 여행자가 참고할 수 있도록 정보 전달 중심의 글로 작성해주세요.
  - 블로거가 직접 숙박해보고 쓴 것처럼 객관적인 정보에 자연스러운 감상을 덧붙여 주세요.
